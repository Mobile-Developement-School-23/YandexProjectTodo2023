...
// MARK: Palette drawing, add gestureRecognizer and get color at point
...
class ColorPickerView: UIView {
    ...
    private func getColorAtPoint(point: CGPoint) -> UIColor {
        ...
        if rect_mainPalette.contains(point) {
            ...
            let extendedGrayColor = UIColor(white: hue, alpha: 1.0)
            guard let sRGBColorSpace = CGColorSpace(name: CGColorSpace.sRGB) else { return UIColor(white: hue, alpha: 1)}
            if let rgbColor = extendedGrayColor.cgColor.converted(to: sRGBColorSpace, intent: .defaultIntent, options: nil) {
                
                return UIColor(cgColor: rgbColor)
            }
            return UIColor(white: hue, alpha: 1)
        }
